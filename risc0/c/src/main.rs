...
use csv::Writer;
use c_core::Outputs;
use c_methods::{VALIDATE_C_ELF, VALIDATE_C_ID};
...
use lang_c::driver::{Config, preprocess};
...
fn main() {
    ...
    let data = preprocess(&config, data_file.as_ref()).unwrap();
    ...
    let outputs = validate_c(&data);
    println!("{:?} is a valid c - {:?}", outputs.hash, outputs.valid);
...
fn validate_c(data: &str) -> Outputs {
    #[cfg(feature = "metrics")]
    ...
    let receipt = prover.prove(env, VALIDATE_C_ELF).unwrap().receipt;
    ...
    assert!(receipt.verify(VALIDATE_C_ID).is_ok());
...
//     #[test]
//     fn main() {
//         let data = include_str!("../res/example.c");
