...
use json_methods::{VALIDATE_JSON_ELF, VALIDATE_JSON_ID};
...
fn main() {
    ...
    let outputs = validate_json(&data);
    println!("{:?} is a valid json - {:?}", outputs.hash, outputs.valid);
...
fn validate_json(data: &str) -> Outputs {
    #[cfg(feature = "metrics")]
    ...
    let prove_info = prover.prove(env, VALIDATE_JSON_ELF).unwrap();
    let receipt = prove_info.receipt;
    println!("Segments: {:?}", prove_info.stats);
    ...
    assert!(receipt.verify(VALIDATE_JSON_ID).is_ok());
